...
export default function App() {
  ...
  useEffect(() => {
      try {
        const resultRaw = localStorage.getItem("pourline-products");
        if (resultRaw) {
          setProducts(JSON.parse(resultRaw));
        }
      } catch (e) {
        // no saved data yet, use defaults
      }
      try {
        const campResultRaw = localStorage.getItem("pourline-campaigns");
        if (campResultRaw) {
          setCampaigns(JSON.parse(campResultRaw));
        }
      } catch (e) {
        // no saved campaigns yet
      }
      try {
        const kdResultRaw = localStorage.getItem("pourline-keydates");
        if (kdResultRaw) {
          setKeyDates(JSON.parse(kdResultRaw));
        }
      } catch (e) {
        // no saved key dates yet
      }
      // Seed national days on first load
      ...
      try {
        const empResultRaw = localStorage.getItem("pourline-employees");
        if (empResultRaw) {
          setEmployees(JSON.parse(empResultRaw));
        }
      } catch (e) {
        // no saved employees yet
      }
      try {
        const postResultRaw = localStorage.getItem("pourline-posts");
        if (postResultRaw) {
          setPosts(JSON.parse(postResultRaw));
        }
      } catch (e) {
        // no saved posts yet
      }
      try {
        const targetsResultRaw = localStorage.getItem("pourline-posttargets");
        if (targetsResultRaw) {
          setPostTargets(JSON.parse(targetsResultRaw));
        }
      } catch (e) {
        // no saved targets yet
      }
      try {
        const dsdResultRaw = localStorage.getItem("pourline-dsdlinks");
        if (dsdResultRaw) {
          setDsdLinks(JSON.parse(dsdResultRaw));
        }
      } catch (e) {
        // no saved dsd links yet
      }
      try {
        const projResultRaw = localStorage.getItem("pourline-projects");
        if (projResultRaw) {
          setProjects(JSON.parse(projResultRaw));
        }
      } catch (e) {
        // no saved projects yet
      }
      try {
        const relResultRaw = localStorage.getItem("pourline-supplierrels");
        if (relResultRaw) {
          setSupplierRels(JSON.parse(relResultRaw));
        }
      } catch (e) {
        // no saved relationships yet
      }
      try {
        const assetResultRaw = localStorage.getItem("pourline-assets");
        if (assetResultRaw) {
          setAssets(JSON.parse(assetResultRaw));
        }
      } catch (e) {
        // no saved assets yet
      }
      try {
        const analyticsResultRaw = localStorage.getItem("pourline-analytics");
        if (analyticsResultRaw) {
          setAnalyticsData(JSON.parse(analyticsResultRaw));
        }
      } catch (e) {}
      try {
        const budgetResultRaw = localStorage.getItem("pourline-budgets");
        if (budgetResultRaw) {
          setBudgets(JSON.parse(budgetResultRaw));
        }
      } catch (e) {}
      setLoaded(true);
  }, []);
  ...
  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-products", JSON.stringify(products)); } catch(e) {}
  }, [products, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-campaigns", JSON.stringify(campaigns)); } catch(e) {}
  }, [campaigns, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-keydates", JSON.stringify(keyDates)); } catch(e) {}
  }, [keyDates, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-employees", JSON.stringify(employees)); } catch(e) {}
  }, [employees, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-posts", JSON.stringify(posts)); } catch(e) {}
  }, [posts, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-posttargets", JSON.stringify(postTargets)); } catch(e) {}
  }, [postTargets, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-dsdlinks", JSON.stringify(dsdLinks)); } catch(e) {}
  }, [dsdLinks, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-projects", JSON.stringify(projects)); } catch(e) {}
  }, [projects, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-supplierrels", JSON.stringify(supplierRels)); } catch(e) {}
  }, [supplierRels, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-assets", JSON.stringify(assets)); } catch(e) {}
  }, [assets, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-analytics", JSON.stringify(analyticsData)); } catch(e) {}
  }, [analyticsData, loaded]);

  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem("pourline-budgets", JSON.stringify(budgets)); } catch(e) {}
  }, [budgets, loaded]);
